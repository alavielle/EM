...
C_TEXT:C284($contenu)
C_OBJECT:C1216($AutreLigneRessource)
...
For ($i; 1; Records in selection:C76([LigneRessource:23]))
	...
	$AutreLigneRessource:=ds:C1482.AutreLigneRessource.query("ID_Ligne = :1"; [LigneRessource:23]NumeroLigne:2)
	If ($AutreLigneRessource.length>0)
		$Contenu:=Replace string:C233($Contenu; "$marge$"; "align-items-end")
		For each ($autre; $AutreLigneRessource)
			$contenu:=$contenu+"<div class=\"form-check m-0\">"
			$contenu:=$contenu+"<label class=\"form-check-label\" >"+$autre.Libelle+"</label>"
			$contenu:=$contenu+"<input name=\"Autre"+String:C10($autre.ID; "00")+"\" class="+Char:C90(34)+$autre.ClassForm+Char:C90(34)+" type="+Char:C90(34)+$autre.Type+Char:C90(34)+" value=''>"
			$contenu:=$contenu+"</div>"
		End for each 
	Else 
