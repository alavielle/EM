...
For ($i; 1; Records in selection:C76([LigneFrais:30]))
	...
	$contenu:=$contenu+"<input type=\"number\" name=\"Frais"+String:C10([LigneFrais:30]NumeroLigne:2; "00")+"\" class=\"form-control text-end\" data-index=\"frais\" onchange=\"totalSaisie(this.name)\">"
	$contenu:=$contenu+"</div>"
	$contenu:=$contenu+"</div>"
